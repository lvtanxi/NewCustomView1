...
public class BottomMenuDialog extends BottomSheetDialogFragment implements View.OnClickListener {
    ...
    private void initRootView() {
        ...
        if (builder.tiems == null || builder.tiems.size() == 0) {
            return;
        }
        if (builder.tiems.size() == 1) {
            View childView = initView(builder.tiems.get(0), 1, false, false);
            childView.setBackgroundDrawable(getDrawableListByType(true, true, true, true));
            return;
        }

        if (builder.tiems.size() == 2) {
            View childView = initView(builder.tiems.get(0), 1, false, true);
            childView.setBackgroundDrawable(getDrawableListByType(true, true, true, true));
            childView = initView(builder.tiems.get(1), 2, false, false);
            childView.setBackgroundDrawable(getDrawableListByType(true, true, true, true));
            return;
        }

        for (int i = 0; i < builder.tiems.size(); i++) {
            View childView = null;
            if (i == 0) {
                childView = initView(builder.tiems.get(0), i + 1, true, false);
                childView.setBackgroundDrawable(getDrawableListByType(true, true, false, false));
                continue;
            }
            if (i == (builder.tiems.size() - 2)) {
                childView = initView(builder.tiems.get(i), i + 1, false, true);
                childView.setBackgroundDrawable(getDrawableListByType(false, false, true, true));
                continue;
            }
            if (i == (builder.tiems.size() - 1)) {
                //false, false, true, true
                childView = initView(builder.tiems.get(i), i + 1, false, false);
                childView.setBackgroundDrawable(getDrawableListByType(true, true, true, true));
                continue;
            }

            childView = initView(builder.tiems.get(i), i + 1, true, false);
    ...
    private View initView(String button, int position, boolean hasBottomLine, boolean hasBottomGap) {
        ...
        LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, dp2px(mContext, 45));
    ...
    public void show() {
        if (!this.isAdded())
            show(builder.fragmentManager, TAG);
    }
    ...
    public void onClick(View v) {
        Object o = v.getTag();
        if (o != null && builder.menuItemClickListener != null) {
            int position = Integer.valueOf(o.toString());
            builder.menuItemClickListener.onMenuItemClick(position, builder.tiems.get(position));
        }
        dismiss();
    }
    ...
    public static class BottomMenuBuilder {
        ...
        public BottomMenuBuilder(FragmentManager fragmentManager) {
        ...
        public BottomMenuBuilder addItems(List<String> tiems) {
            this.tiems = tiems;
            return this;
        }


        public BottomMenuBuilder addOnMenuItemClickListener(OnMenuItemClickListener listener) {
    ...
    public static interface OnMenuItemClickListener {//
        ...
        void onMenuItemClick(int position, String name);
    }
